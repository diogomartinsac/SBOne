...
float linear_speed = 150;
float rotating_speed = 20;
...
void sendMotorPulses() {
  ...
  if(throttle_counter_left_motor > throttle_left_motor_memory){            
    ...
    else REG_WRITE(GPIO_OUT_W1TC_REG, BIT25);                                              
  }
  else if(throttle_counter_left_motor == 1) REG_WRITE(GPIO_OUT_W1TS_REG, BIT26);             
  else if(throttle_counter_left_motor == 2) REG_WRITE(GPIO_OUT_W1TC_REG, BIT26);             
  ...
  if(throttle_counter_right_motor > throttle_right_motor_memory){           
    ...
    else REG_WRITE(GPIO_OUT_W1TS_REG, BIT13);                                              
  }
  else if(throttle_counter_right_motor == 1) REG_WRITE(GPIO_OUT_W1TS_REG, BIT12);            
  else if(throttle_counter_right_motor == 2) REG_WRITE(GPIO_OUT_W1TC_REG, BIT12);            
...
void setup() {
    ...
    timerAttachInterrupt(timer, &sendMotorPulses, true);
...
void loop() {
  ...
  pitch_filtered = pitch * 0.3  + pitch_filtered * 0.7 ;

  if ((pitch_filtered > 40.0  || pitch_filtered < -40) && state_flag == false) state_flag = true;
  if(state == 0 && pitch_filtered > -0.5 && pitch_filtered < 0.5 && state_flag) {
    state = 1;
    move_counter = 0;
  }


  pid_error_temp = pitch_filtered - self_balance_pid_setpoint - pid_setpoint;
  ...
  if(pitch_filtered > 40 || pitch_filtered < -40 || state == 0){
    pid_output = 0;                                             
    pid_i_mem = 0;                                              
    state = 0;                                                  
  ...
  //   move(right);
    // move_counter++;
    // if (move_counter > 500 && move_counter < 3500){
    //   move(forward);
    // }

    // if (move_counter > 3500 && move_counter <= 4000){
    //   move(right);
    // }

    // if (move_counter > 4000 && move_counter < 7000){
    //   move(forward);
    // }
    
    // if (move_counter > 7000 && move_counter <= 7500){
    //   move(right);
    // }

    // if (move_counter > 7500 && move_counter < 10500){
    //   move(forward);
    // }

    // if (move_counter > 10500 && move_counter <= 11000){
    //   move(right);
    // }

    // if (move_counter > 11000 && move_counter <= 14000){
    //   move(forward);
    // }

    // if (move_counter > 14000 && move_counter <= 14500){
    //   move(right);
    //   move_counter = 500;
    // }
  ...
  if(pid_setpoint == 0){                                        
    if(pid_output < 0)
      self_balance_pid_setpoint += 0.0001;      
    if(pid_output > 0)
      self_balance_pid_setpoint -= 0.0001;      
  }
...
void move(int command) {
  switch (command) {
  case left:
    pid_output_left += rotating_speed;
    pid_output_right -= rotating_speed;
    break;
  case right:
    pid_output_left -= rotating_speed;
    pid_output_right += rotating_speed;
  case forward:
    if(pid_setpoint > -10)
      pid_setpoint -= 0.5;
    if(pid_output > linear_speed * -1)
      pid_setpoint -= 0.05;
  case backward:
    if(pid_setpoint < 10)
      pid_setpoint += 0.5;
    if(pid_output < linear_speed)
      pid_setpoint += 0.05;
  case stop:
    if(pid_setpoint > 0.5)
      pid_setpoint -=0.05;
